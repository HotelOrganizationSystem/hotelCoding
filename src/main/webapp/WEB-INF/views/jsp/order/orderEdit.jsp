...
<form class="layui-form">
    <input type="text" hidden name="orderid" id="orderid" value="${order.orderid}">
        ...
        <div class="layui-input-block" style="z-index: 99999">
            <select name="memid" id="memid" lay-verify="" lay-search>
                <c:forEach var="mems" items="${mem}">
                    <c:choose>
                        <c:when test="${mems.memid != order.memid}">
                            <option value="${mems.memid}">${mems.memname}</option>
                        </c:when>
                        <c:otherwise>
                            <option selected value="${mems.memid}">${mems.memname}</option>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
            </select>
        ...
        <div class="layui-input-block" style="z-index: 9999">
            <select  id="rtypeid" lay-verify="" lay-filter="roomt" lay-search >
                <c:forEach var="roomt" items="${roomT}">
                    <option value="${roomt.rtypeid}" <c:if test="${roomt.rtypeid==order.rtypeid}">selected</c:if> >${roomt.rtypename}</option>
                </c:forEach>
            </select>
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block" >
            <select name="roomid" id="roomid" lay-verify="" lay-search lay-filter="roomid">
                <option value="${order.roomid}">${order.roomNum}</option>
                <c:forEach var="rooms" items="${room}">
                        <c:if test="${rooms.isres == 0 and rooms.roomid != order.roomid}">
                            <option value="${rooms.roomid}">${rooms.roomNum}</option>
                        </c:if>
                        <c:if test="${rooms.roomid == order.roomid}">
                            <option selected value="${rooms.roomid}">${rooms.roomNum}</option>
                        </c:if>
                </c:forEach>
            </select>
        </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="radio" name="orderstate" title="未入住" value="未入住" <c:if
                    test="${order.orderstate.equals('未入住')}">checked=""</c:if>>
            <input type="radio" name="orderstate" title="已入住" value="已入住" <c:if
                    test="${order.orderstate.equals('已入住')}">checked=""</c:if>>
            <input type="radio" name="orderstate" title="已退房" value="已退房" <c:if
                    test="${order.orderstate.equals('已退房')}">checked=""</c:if>>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" id="ordertime" name="ordertime" autocomplete="off" placeholder="年份"
                   class="layui-input" value="<fmt:formatDate value="${order.ordertime}"
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="personnum" id="personnum" autocomplete="off" lay-verType="tips"
                   ...
                   placeholder="请输入预计人数"
                   class="layui-input" value="${order.personnum}">
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" id="intime" name="intime" autocomplete="off" placeholder="年份" class="layui-input"
                   value="<fmt:formatDate value="${order.intime}"
                   pattern="yyyy-MM-dd HH:mm:ss"/>">
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" id="outtime" name="outtime" autocomplete="off" placeholder="年份" class="layui-input"
                   value="<fmt:formatDate value="${order.outtime}"
                   pattern="yyyy-MM-dd HH:mm:ss"/>">
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="deposit" id="deposit" autocomplete="off" lay-verType="tips"
                   lay-verify="required|number"
                   placeholder="请输入定金"
                   class="layui-input" value="${order.deposit}">
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="oprice" id="oprice" autocomplete="off" lay-verType="tips"
                   lay-verify="required|number"
                   placeholder="请输入订单价格"
                   class="layui-input" value="${order.OPrice}">
...
<script>
    layui.use(['form', 'layedit', 'laydate'], function(){
        ...
        form.on('submit(demo2)', function(data){
            $.ajax({
                url: '<%=request.getContextPath()%>/order/edit',
                type: 'post',
